<div align="center">

# Drayan Mathematical Series
### by Aryan Goutam

*Olympiad solutions · Original problems · Handouts · Expositions*

[![Problems Solved](https://img.shields.io/badge/Problems%20Solved-28-blue?style=flat-square)](.)
[![Original Problems](https://img.shields.io/badge/Original%20Problems-28-blueviolet?style=flat-square)](.)
[![Handouts](https://img.shields.io/badge/Handouts-4-green?style=flat-square)](.)

</div>

---

## Who I Am

I'm Aryan Goutam — an Olympiad mathematician focused on problem solving, problem **creation**, and mathematical exposition. This repository is my public portfolio: every solution, problem, and handout here is written from scratch and typeset in LaTeX to publication standard.

I'm available for **freelance work** — if you need original problem sets, full solution writeups, or competition handouts, reach out.

---

## Competitive Record

| Competition | Achievement |
|---|---|
| RMO 2025 | Awardee |
| INMO 2026 | Top 500 |
| AMC 12B 2025 | International Rank **3** |
| Fermat Contest | International Rank **7** |
| NMTC 2025 | National Top 200 |
| PW COE Gurukul | Scholar |

---

## What I Offer

### ✍️ Solution Writing
Rigorous, well-motivated writeups for Olympiad problems — AMC / AIME / IOQM / RMO / INMO / ISI / CMI level. Every solution explains the *key idea*, not just the mechanics.

### 🧩 Original Problem Creation
I create original, well-calibrated competition problems across all topics. Problems come with full solutions and difficulty tags. Suitable for mock tests, problem sets, and editorial content.

### 📄 Handouts & Expository Notes
Topic-focused handouts covering theory, worked examples, and practice problems — ready to use for coaching and self-study.

### 🖥️ LaTeX Typesetting
All deliverables come as polished PDFs with clean `.tex` source files included.

---

## Repository Structure

```
Drayan-Mathematical-Series/
│
├── Algebra/
├── Geometry/
├── Number-Theory/
├── Combinatorics/
├── Functional-Equations/
├── Original-Problems/
├── Handouts/
├── Editorials/
└── LaTeX-Resources/
```

---

## Areas of Interest

- Euclidean Geometry
- Number Theory
- Algebra
- Combinatorics
- Functional Equations

---

## Repository Statistics

| Category | Count |
|---|---|
| Problems Solved | 36 |
| Solutions Written | 28 |
| Original Problems Created | 28 |
| Handouts | 4 |
| Editorials | 0 |

---

## Freelance Services

I work with coaching institutes, ed-tech platforms, competition organizers, and individual tutors. Services include:

- **Mock test creation** — full problem sets (RMO / INMO / IOQM / ISI / CMI style) with complete solutions
- **Solution manuals** — batch writeups for existing problem sets
- **Handout writing** — topic notes with theory + problems
- **LaTeX typesetting** — clean formatting of your existing content

📩 **Contact:** [ aryangoutam27112010@gmail.com]  
🔗 **AoPS / LinkedIn:** [https://www.linkedin.com/in/aryan-goutam-448106392 ]

---

## Philosophy

> Mathematics is not about numbers, equations, or algorithms — it is about understanding.  
> Every problem here is an invitation to think, not just compute.

---

<div align="center">
<sub>© Aryan Goutam · Drayan Mathematical Series · All original problems and solutions are the intellectual property of the author.</sub>
</div>
